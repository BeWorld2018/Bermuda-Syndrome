...
void SystemStub_SDL::init(const char *title, int w, int h, bool fullscreen, int screenMode) {
	...
	_window = SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, windowW, windowH, 0 | SDL_WINDOW_RESIZABLE);
	if (_widescreen) {
...
void SystemStub_SDL::handleEvent(const SDL_Event &ev, bool &paused) {
	...
	case SDL_JOYHATMOTION:
		if (ev.jhat.value == SDL_HAT_RIGHTUP)
            {
              _pi.dirMask |= PlayerInput::DIR_UP;
              _pi.dirMask |= PlayerInput::DIR_RIGHT;
             _pi.dirMask &= ~PlayerInput::DIR_DOWN;
	...
	case SDL_JOYBUTTONUP:
		...
		{
			_pi.enter = false; // ENTER X
		}
		if (ev.jbutton.button == 0 )
		{
			_pi.space = false; // SPACE A
		}
		...
		{
			_pi.escape = false; // ESC Y
		}
		if (ev.jbutton.button == 1 )
		{
			_pi.shift = false; // SHIFT B
		}
	...
	case SDL_JOYBUTTONDOWN:
		...
		if (ev.jbutton.button == 2 )
		{
			_pi.enter = true; // ENTER X
		}
		if (ev.jbutton.button == 0 )
		{
			_pi.space = true; // SPACE A
		}
		if (ev.jbutton.button == 5)
		{
			_pi.tab = true; // TAB 
		...
		if (ev.jbutton.button == 3 )
		{
			_pi.escape = true; // ESC Y
		}
		if (ev.jbutton.button == 1 )
		{
			_pi.shift = true; // SHIFT B
		}
		if (ev.jbutton.button == 6 )
		{
			_pi.ctrl = true; // CTRL
